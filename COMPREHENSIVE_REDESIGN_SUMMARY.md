...
- ✅ Current problems (5 major issues identified)
...
## 🏗️ ARCHITECTURE OVERVIEW (What We Analyzed)
...
├── user: FK → User
├── primary_language: str (en, ja, es, etc)
├── secondary_language: str (optional bilingual)
├── output_format: enum (text/audio/both)
├── script_text: text (generated by LLM)
├── audio_file: file (MP3 from TTS)
...
├── cost_per_generation: decimal ← Global cost
├── cooldown_hours: int ← Global cooldown
├── max_generations_per_day: int
├── script_word_limit: int
├── tts_speaking_rate: float
└── ... (30+ fields for complete customization)

StudentGroup (M2M with User)
├── name: str (e.g., "Beginners", "Advanced")
├── description: str
├── users: M2M → User
├── is_active: bool
└── config: O2O → CategoryOverride

CategoryOverride (Per-group settings)
├── category: O2O → StudentGroup
├── default_llm_provider: str (optional override)
├── default_tts_provider: str (optional override)
├── cost_per_generation: decimal (optional override)
├── cooldown_hours: int (optional override)
├── script_word_limit: int (optional override)
└── ... (12 fields that mirror GlobalDefaults)
```
...
Check: Is user in active StudentGroup?
    ├─ YES → Use StudentGroup.config (with fallback to global)
    └─ NO  → Use GlobalPodcastDefaults

For each setting:
    ├─ If override filled → use override
    └─ If override blank → use global default
...
Example:
  ...
  └─ model=gpt-4o-mini ← from global (blank in override)
```
...
Before                              After
─────────────────────────────────────────────────────
Teacher Content Configuration  →  Global Podcast Defaults
User Category                 →  Student Group
User Category Configuration   →  Category Override
```
...
├─ 👥 Users (which users are in this group)
└─ ⚙️ SETTINGS OVERRIDE (what overrides global)
    ├─ AI Model (optional)
    ├─ Cost (optional)
    ├─ Cooldown (optional)
    ├─ Script length (optional)
    └─ [Yellow tip: "blank = use global"]
```
...
After (Logical):
├── Daily Podcasts
├── Global Podcast Defaults (edit once, applies to all)
├── Student Groups (create groups)
└── Category Overrides (manage group overrides)
```

---

## 🎯 PHASE 1 CHANGES (What We Actually Modified)
...
**Impact**: ✅ Zero logic changes, pure naming/documentation
...
- class UserCategoryAdmin → StudentGroupAdmin
- class UserCategoryConfigAdmin → PerCategoryOverrideAdmin
...
+ "💡 Blank = use global, Filled = override"
...
**Impact**: ✅ Zero logic changes, pure UX improvements
...
3. See "⚙️ SETTINGS OVERRIDE" section
4. Fill in Cost = $0.25
...
│ ─────────────────────────────────────────│
│ AI Model   │ gpt4-m │ gpt4     │ gpt4-t  │
│ Cost       │ $0.50  │ $0.25 ✓  │ (use)   │
│ Cooldown   │ 24h    │ 48h ✓    │ 0h ✓    │
│ Word limit │ 1000   │ 800 ✓    │ (use)   │
└──────────────────────────────────────────┘
```
