    ...
    success: function (data) {
      if (data.success) {
        ...
        const courseOptions = data.courses
          ...
          .join("");
        $("#course_select").html(
          "<option>-- Select a course --</option>" + courseOptions
        );
      }
    },
    error: function (err) {
...
async function fetchUserCourses(userId) {
  const response = await fetch(
    `/api/admin/ajax/user-courses/?user_id=${userId}`
  );
...
async function fetchCourseDetails(courseId) {
  const response = await fetch(
    `/api/admin/ajax/course-details/?course_id=${courseId}`
  );
  const data = await response.json();
  return data;
}
...
_Document generated: Implementation Complete_
