...
import styles from './Profile.module.css'; // Using the new CSS module
...
const Profile = () => {
  ...
  }, [token, navigate, fetchProfileCallback, fetchUserScores]);
  ...
  if (!profile) { // If profile fetch failed
    return (
      <div className={`${styles.profilePage} text-center`}>
        ...
        <button onClick={() => navigate('/login')} className={`${styles.sidebarButton} ${styles.publicProfileBtn} mt-6 mx-auto w-auto px-6`}>
            Go to Login
        </button>
      </div>
    );
        ...
        <div className={styles.profileLayout}>
          <aside className={styles.profileSidebar}>
            <div className={styles.sidebarContent}>
              ...
              <Link to={`/guide/${profile.username}`} className={`${styles.sidebarButton} ${styles.publicProfileBtn}`}> View Public Profile </Link>
                      ...
                      {courses.slice(0, displayedCoursesCount).map((course) => (
                        <div key={course.id} className={styles.card}>
                          ...
                          <div className={styles.cardContent}>
                            ...
                            <button onClick={() => navigate(`/courses/${course.permalink}`)} className={styles.cardButton}> View Course </button>
                          </div>
                        </div>
                      ))}
                    </div>
                      ...
                      {lessons.slice(0, displayedLessonsCount).map((lesson) => (
                        <div key={lesson.id} className={styles.card}>
                          ...
                          <div className={styles.cardContent}>
                            ...
                            <button onClick={() => navigate(`/lessons/${lesson.permalink}`)} className={styles.cardButton}> Read Full Lesson </button>
                          </div>
                        </div>
                      ))}
                    </div>
