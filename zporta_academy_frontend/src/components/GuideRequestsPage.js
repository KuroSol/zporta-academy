...
const handleDeny = async (requestId) => {
  ...
  if (!requests.length) return <p>No guide requests available.</p>;
          ...
          <li key={req.id} className="request-item">
            ...
            { user && String(req.guide) === String(user.user_id) && req.status === "pending" && (
              <div className="action-buttons">
                <button onClick={() => handleAccept(req.id)} className="accept-button">
                  Accept
                </button>
                <button onClick={() => handleDeny(req.id)} className="deny-button">
                  Deny
                </button>
              </div>
            )}
