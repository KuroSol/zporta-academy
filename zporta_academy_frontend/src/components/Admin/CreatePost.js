...
import CustomEditor from "../Editor/CustomEditor"; // Adjust path if needed
import apiClient from '../../api'; // <-- ADD apiClient (Adjust path ../../api)
import { AuthContext } from '../../context/AuthContext'; // <-- ADD AuthContext (Adjust path)
import "./CreatePost.css"; // Keep CSS
...
const CreatePost = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (data.permalink) {
                 navigate(`/posts/${data.permalink}`); // Assumes route is /posts/:permalink
             } else {
                 navigate('/explore'); // Fallback navigation
             }
