...
import { 
  ...
  FaBookReader, FaAward, FaEnvelope, FaCog, FaShieldAlt,
  FaBell, FaTrash, FaEye, FaEyeSlash, FaUserCheck
} from "react-icons/fa";
...
const Profile = () => {
  ...
  const [showcaseImageUploadProgress, setShowcaseImageUploadProgress] = useState({
    image1: false,
    image2: false,
    image3: false
  });
  ...
  const searchTags = async (query) => {
    ...
    try {
      const { data } = await apiClient.get(`/tags/?search=${encodeURIComponent(query)}&page_size=10`);
  ...
  const handleAddTag = async (imageNumber, tagName) => {
    ...
    let tag = availableTags.find(t => t.name.toLowerCase() === trimmedName.toLowerCase());
    if (!tag) {
  ...
  }, [token, router, fetchProfileCallback, fetchUserScores, fetchAvailableTags]);
  ...
  }, [activeTab, profile, fetchUserCourses, fetchUserLessons, fetchUserQuizzes, isTeacherOrAdmin]);
  ...
  const handleShowcaseImageUpload = async (imageNumber, file) => {
    ...
    setShowcaseImageUploadProgress(prev => ({ ...prev, [progressKey]: true }));
    setTeacherProfileError('');
    ...
    try {
      ...
      setTeacherProfileError(
        err.response?.data?.detail || `Failed to upload showcase image ${imageNumber}.`
      );
    } finally {
      setShowcaseImageUploadProgress(prev => ({ ...prev, [progressKey]: false }));
    }
  };
  ...
  const handleDeleteShowcaseImage = async (imageNumber) => {
    ...
    try {
      ...
      setTeacherProfileError(
        err.response?.data?.detail || `Failed to delete showcase image ${imageNumber}.`
      );
    }
  };
  ...
  const handleSendInvitation = async (e) => {
    ...
    try {
      ...
      setInvitationSuccess(`Invitation sent successfully! ${data.remaining_invitations_this_month} invitations remaining this month.`);
      setInvitationEmail('');
  ...
  const fetchEnrolledTeachers = async () => {
    ...
    try {
      ...
      const { data } = await apiClient.get('/enrollment/user/');
      
      // Extract unique teachers from enrollments
      const teachers = {};
      
      for (const enrollment of data) {
        // Get course details to find teacher
        if (enrollment.course && enrollment.course.id) {
          try {
            const courseResponse = await apiClient.get(`/courses/${enrollment.course.id}/`);
            ...
            if (course.teacher && course.teacher.id) {
              ...
              if (!teachers[teacherId]) {
                teachers[teacherId] = {
                  ...
                  display_name: course.teacher.display_name || course.teacher.username,
                  profile_picture_url: course.teacher.profile_picture_url,
                  course_count: 0
                };
              }
              teachers[teacherId].course_count++;
            }
          } catch (err) {
            console.error(`Error fetching course ${enrollment.course.id}:`, err);
          }
        }
      }
      
      setEnrolledTeachers(Object.values(teachers));
    } catch (err) {
  ...
  const fetchMyStudents = async () => {
    ...
    try {
      // Get courses where I'm the teacher
      ...
      for (const course of data) {
        try {
          const enrollments = await apiClient.get(`/enrollment/course/${course.id}/`);
            ...
            if (enrollment.student && enrollment.student.id) {
              ...
              if (!students[studentId]) {
                students[studentId] = {
                  ...
                  display_name: enrollment.student.display_name || enrollment.student.username,
                  profile_picture_url: enrollment.student.profile_picture_url,
                  course_count: 0,
                  courses: []
                };
              }
              students[studentId].course_count++;
              students[studentId].courses.push(course.title);
            }
          });
        } catch (err) {
      ...
      setMyStudents(Object.values(students));
    } catch (err) {
  ...
  const handleDeleteAccount = async () => {
    ...
    const userInput = prompt(`This action cannot be undone! Type "${confirmText}" to permanently delete your account:`);
  ...
  return (
    <div className={styles.profilePage}>
      <div className={styles.profileContainer}>
                ...
                {!isTeacherOrAdmin && (
                  <div className={styles.infoSection}>
                    ...
                    {editingBio ? (
                      ...
                      <p className={`${styles.infoValue} ${profile.bio ? '' : 'italic'}`}>
            ...
            <nav className={styles.tabNavigation}>
              ...
              {isTeacherOrAdmin && (
                <button 
                  ...
                  className={`${styles.tabButton} ${activeTab === 'teacher' ? styles.tabButtonActive : ''} flex items-center gap-2`}
              ...
              <button 
                ...
                className={`${styles.tabButton} ${activeTab === 'settings' ? styles.tabButtonActive : ''} flex items-center gap-2`}
                ...
                <div className={teacherStyles.teacherDashboard}>
                  <div className={teacherStyles.dashboardContainer}>
                    <div className={teacherStyles.dashboardHeader}>
                      ...
                      <p>Manage your teaching profile and invite other educators to join Zporta Academy</p>
                    </div>
                    ...
                    <div className={teacherStyles.profileCard}>
                      ...
                      <div className={teacherStyles.formGroup}>
                        <label className={teacherStyles.formLabel}>Teaching Bio</label>
                        <textarea
                          value={teacherBio}
                        ...
                        />
                        <p className={teacherStyles.formHint}>💼 This will be displayed on your public teacher profile</p>
                      </div>
                      
                      <div className={teacherStyles.formGroup}>
                        <label className={teacherStyles.formLabel}>Specialization / Subjects</label>
                        <input
                          type="text"
                        ...
                        />
                        <p className={teacherStyles.formHint}>🎓 Your areas of expertise</p>
                      </div>
                      
                      <div className={teacherStyles.gridCols3}>
                        <div className={teacherStyles.formGroup}>
                          <label className={teacherStyles.formLabel}>Website</label>
                          <input
                            type="url"
                        ...
                        <div className={teacherStyles.formGroup}>
                          <label className={teacherStyles.formLabel}>LinkedIn</label>
                          <input
                            type="url"
                        ...
                        <div className={teacherStyles.formGroup}>
                          <label className={teacherStyles.formLabel}>Twitter/X</label>
                          <input
                            type="url"
                      ...
                      <div className={teacherStyles.formGroup}>
                        <label className={teacherStyles.formLabel}>Showcase Gallery (Portfolio)</label>
                        <p className={teacherStyles.formHint}>🖼️ Upload up to 3 images to showcase your work, classroom, or achievements. These appear on your public profile.</p>
                        
                        <div className={teacherStyles.showcaseGallery}>
                          {/* Showcase Image 1 */}
                          <div className={teacherStyles.showcaseImageSlot}>
                            {showcaseImage1 ? (
                              <div className={teacherStyles.showcaseImagePreview}>
                              ...
                              <label className={teacherStyles.showcaseImageUpload}>
                                {showcaseImageUploadProgress.image1 ? (
                                  <span>Uploading...</span>
                                ) : (
                                  <>
                                    <span className={teacherStyles.uploadIcon}>📸</span>
                                    <span>Upload Image 1</span>
                                    <input
                                      type="file"
                                      accept="image/*"
                                      onChange={(e) => handleShowcaseImageUpload(1, e.target.files[0])}
                                      style={{ display: 'none' }}
                                    />
                                  </>
                                )}
                              ...
                              <div className={teacherStyles.showcaseMeta}>
                                <div className={teacherStyles.captionGroup}>
                                  <input
                                    ...
                                    onChange={(e) => setShowcaseCaption1(e.target.value)}
                                    onBlur={() => handleSaveShowcaseCaption(1, showcaseCaption1)}
                                    placeholder="Add caption (max 200 characters)"
                                    maxLength={200}
                                    className={teacherStyles.captionInput}
                                  />
                                  <span className={teacherStyles.charCount}>{showcaseCaption1.length}/200</span>
                                </div>
                                
                                <div className={teacherStyles.tagSelector}>
                                  <label className={teacherStyles.tagLabel}>Tags:</label>
                                  ...
                                  <div className={teacherStyles.selectedTags}>
                                    {showcaseTags1.map(tagId => {
                                      ...
                                      return tag ? (
                                        <span key={tagId} className={teacherStyles.tagChip}>
                                          {tag.name}
                                          <button
                                            type="button"
                                            onClick={() => handleRemoveTag(1, tagId)}
                                  ...
                                  <div className={teacherStyles.tagInputWrapper}>
                                    <input
                                      type="text"
                                      value={tagInput1}
                                      onChange={(e) => handleTagInputChange(1, e.target.value)}
                                      onKeyDown={(e) => {
                                        if (e.key === 'Enter') {
                                          e.preventDefault();
                                    ...
                                    {showSuggestions1 && tagSuggestions1.length > 0 && (
                                      <div className={teacherStyles.tagSuggestions}>
                                        {tagSuggestions1.map(tag => (
                                          <div
                                            key={tag.id}
                                            className={teacherStyles.tagSuggestion}
                                            onClick={() => handleAddTag(1, tag.name)}
                                          >
                                            {tag.name}
                                          </div>
                                        ))}
                                    ...
                                    {showSuggestions1 && tagInput1.trim() && !tagSuggestions1.find(t => t.name.toLowerCase() === tagInput1.toLowerCase()) && (
                                      <div className={teacherStyles.tagSuggestions}>
                                        <div
                                ...
                                {showcaseImageUploadProgress.image2 ? (
                                  ...
                                  <>
                                    ...
                                    <input
                                      type="file"
                                      accept="image/*"
                                      onChange={(e) => handleShowcaseImageUpload(2, e.target.files[0])}
                                      style={{ display: 'none' }}
                                    />
                                  </>
                                )}
                              ...
                              <div className={teacherStyles.showcaseMeta}>
                                <div className={teacherStyles.captionGroup}>
                                  <input
                                    ...
                                    onChange={(e) => setShowcaseCaption2(e.target.value)}
                                    onBlur={() => handleSaveShowcaseCaption(2, showcaseCaption2)}
                                    placeholder="Add caption (max 200 characters)"
                                    maxLength={200}
                                    className={teacherStyles.captionInput}
                                  />
                                  <span className={teacherStyles.charCount}>{showcaseCaption2.length}/200</span>
                                </div>
                                ...
                                <div className={teacherStyles.tagSelector}>
                                    ...
                                    {showcaseTags2.map(tagId => {
                                      ...
                                      return tag ? (
                                        <span key={tagId} className={teacherStyles.tagChip}>
                                          ...
                                          <button
                                            type="button"
                                            onClick={() => handleRemoveTag(2, tagId)}
                                            className={teacherStyles.tagRemove}
                                          >
                                            ×
                                          </button>
                                  ...
                                  <div className={teacherStyles.tagInputWrapper}>
                                    <input
                                      type="text"
                                      value={tagInput2}
                                      onChange={(e) => handleTagInputChange(2, e.target.value)}
                                      onKeyDown={(e) => {
                                        if (e.key === 'Enter') {
                                          e.preventDefault();
                                    ...
                                    {/* Autocomplete Suggestions */}
                                    {showSuggestions2 && tagSuggestions2.length > 0 && (
                                      <div className={teacherStyles.tagSuggestions}>
                                        {tagSuggestions2.map(tag => (
                                          <div
                                            key={tag.id}
                                            className={teacherStyles.tagSuggestion}
                                            onClick={() => handleAddTag(2, tag.name)}
                                          >
                                            {tag.name}
                                          </div>
                                        ))}
                                    ...
                                    {/* Create new tag option */}
                                    {showSuggestions2 && tagInput2.trim() && !tagSuggestions2.find(t => t.name.toLowerCase() === tagInput2.toLowerCase()) && (
                                      <div className={teacherStyles.tagSuggestions}>
                                        <div
                                          className={`${teacherStyles.tagSuggestion} ${teacherStyles.createNew}`}
                                ...
                                {showcaseImageUploadProgress.image3 ? (
                                  ...
                                  <>
                                    ...
                                    <input
                                      type="file"
                                      accept="image/*"
                                      onChange={(e) => handleShowcaseImageUpload(3, e.target.files[0])}
                                      style={{ display: 'none' }}
                                    />
                                  </>
                                )}
                              ...
                              <div className={teacherStyles.showcaseMeta}>
                                <div className={teacherStyles.captionGroup}>
                                  <input
                                    ...
                                    onChange={(e) => setShowcaseCaption3(e.target.value)}
                                    onBlur={() => handleSaveShowcaseCaption(3, showcaseCaption3)}
                                    placeholder="Add caption (max 200 characters)"
                                    maxLength={200}
                                    className={teacherStyles.captionInput}
                                  />
                                  <span className={teacherStyles.charCount}>{showcaseCaption3.length}/200</span>
                                </div>
                                ...
                                <div className={teacherStyles.tagSelector}>
                                    ...
                                    {showcaseTags3.map(tagId => {
                                      ...
                                      return tag ? (
                                        <span key={tagId} className={teacherStyles.tagChip}>
                                          ...
                                          <button
                                            type="button"
                                            onClick={() => handleRemoveTag(3, tagId)}
                                            className={teacherStyles.tagRemove}
                                          >
                                            ×
                                          </button>
                                  ...
                                  <div className={teacherStyles.tagInputWrapper}>
                                    <input
                                      type="text"
                                      value={tagInput3}
                                      onChange={(e) => handleTagInputChange(3, e.target.value)}
                                      onKeyDown={(e) => {
                                        if (e.key === 'Enter') {
                                          e.preventDefault();
                                    ...
                                    {/* Autocomplete Suggestions */}
                                    {showSuggestions3 && tagSuggestions3.length > 0 && (
                                      <div className={teacherStyles.tagSuggestions}>
                                        {tagSuggestions3.map(tag => (
                                          <div
                                            key={tag.id}
                                            className={teacherStyles.tagSuggestion}
                                            onClick={() => handleAddTag(3, tag.name)}
                                          >
                                            {tag.name}
                                          </div>
                                        ))}
                                    ...
                                    {/* Create new tag option */}
                                    {showSuggestions3 && tagInput3.trim() && !tagSuggestions3.find(t => t.name.toLowerCase() === tagInput3.toLowerCase()) && (
                                      <div className={teacherStyles.tagSuggestions}>
                                        <div
                                          className={`${teacherStyles.tagSuggestion} ${teacherStyles.createNew}`}
                      ...
                      <div className={teacherStyles.autoSaveNotice}>
                        <p><strong>Auto-save enabled:</strong> Changes are saved automatically when you click away from each field</p>
                      </div>
                    </div>
                    ...
                    <div className={teacherStyles.invitationCard}>
                      ...
                      {!canInvite ? (
                        <div className={teacherStyles.lockedCard}>
                          ...
                          <p>You need special permission to invite other teachers to the platform.</p>
                          <p className="small">Please contact an administrator to request invitation privileges.</p>
                        </div>
                      ) : (
                        <>
                          <div className={teacherStyles.statsBar}>
                            <div className={teacherStyles.statBox}>
                              ...
                              <div className={teacherStyles.statValue}>{remainingInvitations}</div>
                            </div>
                            <div className={teacherStyles.statBox}>
                              <p>Sent This Month</p>
                              <div className={teacherStyles.statValue}>{3 - remainingInvitations}</div>
                            </div>
                          </div>
                          ...
                          <form onSubmit={handleSendInvitation} className={teacherStyles.invitationForm}>
                            <div className={teacherStyles.formRow}>
                              <div className={teacherStyles.formGroup}>
                                <label className={teacherStyles.formLabel}>Email Address *</label>
                                <input
                                  type="email"
                              ...
                              >
                                {remainingInvitations === 0 ? '🚫 No Invitations Left' : '📨 Send Invitation'}
                              </button>
                            </div>
                            
                            <div className={teacherStyles.formGroup}>
                              <label className={teacherStyles.formLabel}>Personal Message (Optional)</label>
                              <textarea
                              ...
                              />
                              <p className={teacherStyles.formHint}>💌 Make it personal - invitees are more likely to accept!</p>
                            </div>
                          </form>
                          ...
                          <div className={teacherStyles.historySection}>
                            ...
                            {invitationsLoading ? (
                              ...
                              <div className={teacherStyles.invitationList}>
                                {invitations.map((inv) => (
                                  <div key={inv.id} className={teacherStyles.invitationItem}>
                                    <div className={teacherStyles.invitationInfo}>
                                      <div className={teacherStyles.invitationEmail}>{inv.invitee_email}</div>
                                      <div className={teacherStyles.invitationMeta}>
                                        <span className={`${teacherStyles.statusBadge} ${
                                          inv.status === 'accepted' ? teacherStyles.statusAccepted : 
                                          inv.status === 'expired' ? teacherStyles.statusExpired : 
                                          inv.status === 'cancelled' ? teacherStyles.statusCancelled : 
                                        ...
                                        <span className={teacherStyles.invitationDate}>
                                          Sent {new Date(inv.created_at).toLocaleDateString()}
                                        </span>
                                        {inv.is_expired && !['accepted', 'cancelled'].includes(inv.status) && (
                                          <span className={teacherStyles.invitationDate} style={{color: '#dc2626', fontWeight: '600'}}>
                                            Expired
                                          </span>
                                        )}
                                      </div>
                              ...
                              <div className={teacherStyles.emptyState}>
                                ...
                                <p className="small">Start inviting talented teachers to join the platform!</p>
                              </div>
                            )}
                          </div>
                        </>
                  ...
                  <div className={teacherStyles.settingsContainer}>
                      ...
                      <div className={teacherStyles.settingsGroup}>
                        ...
                        <label className={teacherStyles.settingToggle}>
                          <div>
                            ...
                            <p className={teacherStyles.settingDescription}>
                              Receive educational content from teachers you follow
                            </p>
                          </div>
                          <input
                            type="checkbox"
                            checked={mailMagazineEnabled}
                            onChange={(e) => setMailMagazineEnabled(e.target.checked)}
                            className={teacherStyles.toggle}
                          />
                        </label>
                          ...
                          <div className={teacherStyles.nestedSettings}>
                            ...
                            {loadingTeachers ? (
                              <div style={{textAlign: 'center', padding: '20px'}}>
                                <FaSpinner className={teacherStyles.spinner} /> Loading teachers...
                              </div>
                            ) : enrolledTeachers.length > 0 ? (
                              <div className={teacherStyles.teacherList}>
                                {enrolledTeachers.map(teacher => (
                                  <div key={teacher.id} className={teacherStyles.userListItem}>
                                    <div style={{display: 'flex', alignItems: 'center', gap: '12px', flex: 1}}>
                                      ...
                                      />
                                      <div>
                                        <div className={teacherStyles.userName}>{teacher.display_name}</div>
                                        <div className={teacherStyles.userMeta}>
                                          @{teacher.username} • {teacher.course_count} course{teacher.course_count > 1 ? 's' : ''}
                                        </div>
                                      </div>
                                    </div>
                                    <button
                                      onClick={() => router.push(`/guide/${teacher.username}`)}
                    ...
                    {isTeacherOrAdmin && (
                      <div className={teacherStyles.settingsCard}>
                        ...
                        <div className={teacherStyles.settingsGroup}>
                          {loadingStudents ? (
                            <div style={{textAlign: 'center', padding: '20px'}}>
                              <FaSpinner className={teacherStyles.spinner} /> Loading students...
                            </div>
                          ) : myStudents.length > 0 ? (
                            <div className={teacherStyles.studentList}>
                              {myStudents.map(student => (
                                <div key={student.id} className={teacherStyles.userListItem}>
                                  <div style={{display: 'flex', alignItems: 'center', gap: '12px', flex: 1}}>
                                    <img 
                                    ...
                                    />
                                    <div>
                                      <div className={teacherStyles.userName}>{student.display_name}</div>
                                      <div className={teacherStyles.userMeta}>
                                        @{student.username} • Enrolled in {student.course_count} course{student.course_count > 1 ? 's' : ''}
                                      </div>
                                    </div>
                                  </div>
                                  <button
                                    onClick={() => router.push(`/guide/${student.username}`)}
                                    className={teacherStyles.viewProfileBtn}
                                  >
                                    <FaEye /> View Profile
                                  </button>
                            ...
                            <p className={teacherStyles.emptyText}>
                              No students enrolled in your courses yet. Keep creating great content!
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    <div className={teacherStyles.saveButtonContainer}>
                      ...
                      >
                        {settingsSaving ? (
                          <>
                            <FaSpinner className={teacherStyles.spinner} /> Saving...
                          </>
                        ) : (
                          <>💾 Save Settings</>
                        )}
                    ...
                    {/* Danger Zone */}
                    <div className={`${teacherStyles.settingsCard} ${teacherStyles.dangerZone}`}>
                      <div className={teacherStyles.cardHeader}>
                        <div className={teacherStyles.cardIcon}>
                          <FaTrash />
                        </div>
        ...
        {showDeactivateModal && (
          <div className={teacherStyles.modal} onClick={() => setShowDeactivateModal(false)}>
            <div className={teacherStyles.modalContent} onClick={(e) => e.stopPropagation()}>
              <h3>Deactivate Account</h3>
              <p>Are you sure you want to deactivate your account? You can reactivate it by logging in again.</p>
