...
import listStyles from '@/styles/DiaryList.module.css';
// modal + tokens are global via globals.css
import styles from '@/styles/DiaryMentions.module.css';
...
const DiaryMentions = () => {
 ...
 // ----------------------------------------------------------------
    ...
    for (let i = 1; i <= totalPages; i++) {
      pages.push(
        <button
          key={i}
          onClick={() => goToPage(i)}
          className={i === currentPage ? listStyles['active'] : ''}>
          {i}
        </button>
      );
    }
  ...
  return (
    <div className={listStyles['diary-list-container']}>
      <h2 className="diary-list-title">Your Mentions</h2>
      {/* Search Input */}
      <div className={listStyles['search-container']}>
        <input
          type="text"
      ...
      {filteredMentions.length === 0 ? (
        ...
        <>
          <div className={listStyles['table-responsive']}>
            <table className={listStyles['diary-table']}>
              <thead>
                <tr>
                <th>#</th>
                <th>Author</th>     {/* NEW */}
                ...
                {currentMentions.map((mention, index) => {
                  ...
                  return (
                    <tr key={mention.id}>
                      ...
                      <td>
                        <div className={listStyles['note-text']} dangerouslySetInnerHTML={{ __html: displayedText }} />
                        {!searchTerm &&
                          getPlainText(mention.note_text).trim().split(/\s+/)
                            .length > 4 && (
                            <button className={listStyles['see-more-btn']} onClick={() => openViewModal(mention)}>
                      ...
                      <td>
                        ...
                        <div className={listStyles['diary-action-buttons']}>
                          <button
                            className={listStyles['diary-action-btn']}
