...
import { QuizMedia, HintBlock } from "./common/QuizContent";
...
const QuizPage = ({
  ...
  const fetchQuiz = useCallback(async () => {
    ...
    try {
      ...
      if (typeof window !== "undefined")
        console.log("GET", (apiClient.defaults?.baseURL || "") + url);
      ...
      if (
        typeof fetchedQuizData.title !== "string" ||
        !Array.isArray(fetchedQuizData.questions)
      ) {
      ...
      const processedQuestions = fetchedQuizData.questions.map((q) => {
        ...
        processedQ.question_data = {
          items: Array.isArray(q.question_data?.items)
            ? q.question_data.items
            : [],
          dropZones: Array.isArray(q.question_data?.dropZones)
            ? q.question_data.dropZones
            : [],
        };
        if (typeof q.correct_options === "string") {
          try {
            ...
            console.error(
              `Error parsing correct_options for question ${q.id}`,
              e
            );
        ...
        if (q.question_type === "dragdrop") {
          if (
            !q._fill_blank ||
            typeof q._fill_blank.sentence !== "string" ||
            !Array.isArray(q._fill_blank.words) ||
            !Array.isArray(q._fill_blank.solutions)
          ) {
            console.error(
              `Invalid _fill_blank for dragdrop question ${q.id}:`,
              q._fill_blank
            );
  ...
  useEffect(() => {
    ...
    quizStartTime.current = Date.now(); // Start timer
    apiClient
  ...
  useEffect(() => {
    if (quizData?.id) {
      ...
      setStatsError(null);
      apiClient
        .get(`/analytics/quizzes/${quizData.id}/detailed-statistics/`)
        .then((res) => {
          const data = res.data;
          setQuizStats({
  ...
  // Update browser URL when navigating between questions
  useEffect(() => {
    if (currentQuestion?.permalink) {
      const newUrl = `/quizzes/q/${currentQuestion.permalink}`;
      window.history.replaceState(null, "", newUrl);
    }
  }, [currentQuestion?.permalink]);
  ...
  const handleSubmitAnswer = async (
    ...
    if (submittedAnswers[questionId]) return;
    const answer =
      answerOverride !== null ? answerOverride : userAnswers[questionId];
    ...
    if (question.question_type === "mcq") {
      ...
      const correctSet = new Set(
        Array.isArray(question.correct_options) ? question.correct_options : []
      );
      ...
      const correctAnswerArray = Array.isArray(question.correct_options)
      ...
      if (
        backendSolutions.length === 0 &&
        Object.keys(studentMap).length === 0
      ) {
        isCorrect = true;
      } else {
        isCorrect = backendSolutions.every((sol) => {
          const wordInBank = (question._fill_blank.words || []).find(
            (w) => w.id === sol.correct_word
          );
          if (!wordInBank) return false;
          const wordIndexInBank = (question._fill_blank.words || []).findIndex(
            (w) => w.id === sol.correct_word
          );
          const correctFrontendWordId = `item_${
            question.temp_id || question.id
          }_${wordIndexInBank}`;
          const frontendZoneIdForSolution = `zone_${
            question.temp_id || question.id
          }_${sol.slot_index}`;
          return (
            studentMap[frontendZoneIdForSolution] === correctFrontendWordId
          );
        });
      }
    ...
    if (token) {
      try {
        ...
        const selectedText =
          cq.question_type === "mcq" && answer >= 1 && answer <= 4
  ...
  if (!quizData || !questions.length) {
    return (
      <ErrorDisplay message="Quiz data is unavailable or has no questions." />
    );
  }
            ...
            {mcqOptions.map((opt) => (
              <button
                ...
                className={`${styles.optionButton}
                            ${
                              userAnswers[currentQuestionId] === opt.index + 1
                                ? styles.selected
                                : ""
                            }
                            ${
                              isAnswerSubmitted &&
                              currentFeedback?.correctValue === opt.index + 1
                                ? styles.correct
                                : ""
                            }
                            ${
              ...
              >
                ...
                {isAnswerSubmitted &&
                  currentFeedback?.correctValue === opt.index + 1 && (
                    <CheckCircle
                      size={20}
                      className={styles.optionFeedbackIconCorrect}
                    />
                  )}
                {isAnswerSubmitted &&
                  userAnswers[currentQuestionId] === opt.index + 1 &&
                  !currentFeedback?.isCorrect && (
                    <XCircle
                      size={20}
                      className={styles.optionFeedbackIconIncorrect}
                    />
                  )}
              </button>
            ))}
        ...
        return (
          <div className={`${styles.optionsGrid} ${styles.multiOptions}`}>
            {mcqOptions.map((opt) => {
              ...
              const isCorrectOption = currentFeedback?.correctValue?.includes(
                opt.index + 1
              );
              return (
                <label
                  ...
                  className={`${styles.optionLabel} ${styles.optionButton}
                            ...
                            ${
                              isAnswerSubmitted && isChecked && !isCorrectOption
                                ? styles.incorrect
                                : ""
                            }
                            ${isAnswerSubmitted ? styles.disabled : ""}
                ...
                >
                  <input
                    ...
                    checked={isChecked}
                    onChange={(e) =>
                      handleMultiAnswer(opt.index, e.target.checked)
                    }
                    disabled={isAnswerSubmitted}
                  />
                  <div className={styles.optionContent}>
          ...
          <div className={styles.shortAnswerArea}>
            ...
            {currentQuestion.allow_speech_to_text && (
              <SpeechToTextInput
                onTranscriptReady={(transcript) =>
                  handleAnswerChange(currentQuestionId, transcript)
                }
                disabled={isAnswerSubmitted}
              />
            )}
            {!isAnswerSubmitted && (
              <button
                ...
                className={`${styles.btn} ${styles.btnSubmitAnswer}`}
                onClick={() => handleSubmitAnswer(currentQuestionId)}
                disabled={!userAnswers[currentQuestionId]?.trim()}
              >
                Submit Answer
              </button>
            )}
      ...
      case "dragdrop":
        ...
        return (
          <FillInTheBlanksQuestion
            ...
            disabled={isAnswerSubmitted}
            onSubmit={(filledMap) =>
              handleSubmitAnswer(currentQuestionId, filledMap)
            }
            submittedAnswer={userAnswers[currentQuestionId]}
  ...
  return (
    <>
        ...
        <div className={styles.quizContainer}>
            ...
            <button
              ...
              onClick={() => {
                const link = currentQuestion?.permalink
                  ...
                  : `${window.location.origin}/quizzes/${permalink}/`;
                if (navigator.share) {
                  navigator
                    .share({ title: quizData.title, url: link })
                    .catch(() => {});
                } else {
                  navigator.clipboard
                    .writeText(link)
          ...
          <div className={styles.attachmentBadges}>
            {quizData.lesson_title ? (
              <Link
                href={`/lessons/${quizData.lesson_permalink}`}
                className={styles.attachmentBadge}
                title={`Part of lesson: ${quizData.lesson_title}`}
              >
                📖 {quizData.lesson_title}
              </Link>
              ...
              <span
                className={styles.attachmentBadge}
                title={`Part of course: ${quizData.course_title}`}
              >
                🎓 {quizData.course_title}
              </span>
            ) : (
              <span
                className={styles.attachmentBadge + " " + styles.standaloneQuiz}
          ...
          {isOwner && (
            <div className={styles.quizActions}>
              <button
                onClick={() =>
                  (window.location.href = `/admin/create-quiz/${quizData.id}`)
                }
                title="Edit Quiz"
            ...
            <div className={styles.progressBar}>
              <div
                ...
                style={{
                  width: `${
                    totalQuestions > 0
                      ? ((safeCurrentIndex + 1) / totalQuestions) * 100
                      : 0
                  }%`,
                }}
              ></div>
        ...
        {statsError ? (
          ...
          <div className={styles.statsSummary}>
            <StatItem
              icon={<Users size={20} />}
              label="Participants"
              value={quizStats.uniqueParticipants}
            />
            <StatItem
              icon={<UserCheck size={20} />}
              label="Finishers"
              value={quizStats.uniqueFinishers}
            />
            <StatItem
              icon={<ListChecks size={20} />}
              label="Total Answers"
              value={quizStats.totalAnswersSubmitted}
            />
            <StatItem
              icon={<CheckSquare size={20} />}
              label="Correct"
              value={quizStats.totalCorrectAnswers}
            />
            <StatItem
            ...
            <StatItem
              ...
              label="Correctness"
              value={`${quizStats.overallCorrectness}%`}
            />
          </div>
        ) : (
          ...
          <>
            <div className={styles.questionCard}>
              <QuizMedia
                ...
                containerClass={styles.questionMediaArea}
            ...
            <div className={styles.answerArea}>{renderAnswerArea()}</div>
        ...
        <div className={styles.navigation}>
          ...
          {safeCurrentIndex === totalQuestions - 1 && isAnswerSubmitted ? (
            <div className={styles.completionMessage}>
              ...
              <button
                onClick={() => {
                  // Record total time spent on quiz
                  if (quizStartTime.current && quizData?.id) {
                    const totalTimeMs = Date.now() - quizStartTime.current;
                    apiClient
                      .post(`/quizzes/${quizData.id}/record-completion/`, {
                ...
                className={`${styles.btn} ${styles.resultsButton}`}
              >
                Back to Dashboard
              </button>
            </div>
      ...
      <ShareQuizModal
        ...
        quizLink={
          currentQuestion?.permalink
            ? `https://zportaacademy.com/quizzes/q/${currentQuestion.permalink}`
            : `https://zportaacademy.com/quizzes/${permalink}/`
        }
      />
    </div>
    </>
