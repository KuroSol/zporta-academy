    ...
    <div className={`${styles.gridItemCard} ${styles.skeleton}`}>
      ...
      <div className={styles.skeletonInfo}>
        <div
          ...
          style={{ width: "80%", height: "1rem", marginBottom: "0.5rem" }}
        ></div>
        <div
          className={styles.skeletonText}
...
const Explorer = () => {
  ...
  const tabs = [
    ...
    { key: "lessons", label: t("nav.lessons"), path: "/lessons/" },
    { key: "quizzes", label: t("nav.quizzes"), path: "/quizzes/" },
    { key: "guides", label: t("nav.guides"), path: "/users/guides/" },
  ];
    ...
    if (
      ...
      return (
        <div className={`${styles.messageContainer} ${styles.infoMessage}`}>
          {t("exploration.noResultsFor", { term: searchTerm })}
        </div>
      );
    }
          ...
          ([category, results]) =>
            results.length > 0 && (
              <section key={category}>
                ...
                <div
                  className={`${styles.gridContainer} ${getCategoryLayoutClass(
                    category
                  )}`}
                >
                  {results.map((item) => {
                    if (category === "quizzes") {
                      return (
                        <QuizCard key={`search-quiz-${item.id}`} quiz={item} />
                      );
                    }
                    return (
                      <ItemCard
                        key={`search-${category}-${item.id}`}
                        item={item}
                        type={category}
                        t={t}
                      />
  ...
  const renderTabContent = () => {
    ...
    if (loading) {
      return (
        <div className={`${styles.gridContainer} ${gridLayoutClass}`}>
          {Array.from({ length: activeTab === "quizzes" ? 3 : 12 }).map(
            (_, index) =>
              activeTab === "quizzes" ? (
                <QuizCard
                  key={`skeleton-quiz-${index}`}
                  quiz={{}}
                  isLoading={true}
                />
              ) : (
    ...
    if (items.length === 0) {
      ...
      return (
        <div className={`${styles.messageContainer} ${styles.infoMessage}`}>
          {t("exploration.noItems", { type: activeLabel })}
        </div>
      );
    }
