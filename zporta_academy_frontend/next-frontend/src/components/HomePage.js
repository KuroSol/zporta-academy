...
import {
  ...
  FaCheckCircle, FaTimesCircle, FaEllipsisH, FaBookOpen, FaSearch,
  FaBrain, FaBook, FaPencilAlt, FaUserFriends, FaClipboardList, FaFilter
} from "react-icons/fa";
...
const HomePage = () => {
    ...
    useEffect(() => {
        const handleOutsideClick = (event) => {
            if (searchHexagonRef.current && !searchHexagonRef.current.contains(event.target)) {
    ...
    const hasAnyResults = searchResults && Object.values(searchResults).some(arr => Array.isArray(arr) && arr.length > 0);

    return (
        <div className={styles.pageContainer}>
            <section className={styles.hexagonSection} ref={hexagonSectionRef}>

                <div className={`${styles.hexagonContainer} ${isSearchExpanded ? styles.searchExpanded : ''}`}>
                    <Link href="/study/dashboard" className={`${styles.hexagon} ${styles.hex1}`}>
                    ...
                    <div
                        ...
                        className={`${styles.hexagon} ${styles.hex4} ${styles.searchHexagon} ${isSearchExpanded ? styles.expanded : ''}`}
                    ...
                    >
                        ...
                        {isSearchExpanded && searchTerm.trim() && (
                            <div className={styles.searchDropdown}>
                                ...
                                {!isSearching && hasAnyResults && Object.entries(searchResults).map(([category, results]) => (
                                    results.length > 0 && <SearchBlock key={category} title={category} items={results} />
                                ))}
                            </div>
                        )}
                    </div>
                    <Link href="/learn/?tab=quizzes" className={`${styles.hexagon} ${styles.hex5}`}>
                        <FaBrain size="2em" className={styles.icon} />
                        <span className={styles.label}>Quizzes</span>
                    </Link>
                    <Link href="/learn/?tab=guides" className={`${styles.hexagon} ${styles.hex6}`}>
                    ...
                    {showStudentSections && (
                        <>
                            <div className={styles.dashboardCard}>
                                        ...
                                        {enrolledCourses.map(enrollment => (
                                            enrollment?.course && (
                                                <div
                                                    ...
                                                    onClick={() => router.push(`/courses/enrolled/${enrollment.id}`)}
                                                    role="button" tabIndex={0}
                                                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') router.push(`/courses/enrolled/${enrollment.id}`); }}
                                                    aria-label={`Go to course: ${enrollment.course.title || 'Untitled Course'}`}
                                                >
                                                    {enrollment.course.cover_image ? (
                                                       <img src={enrollment.course.cover_image} alt="" className={styles.enrolledPreviewThumb} onError={(e) => { e.currentTarget.style.display='none'; }}/>
                                                    ) : (
                                ...
                                <button className={styles.cardActionButton} onClick={() => router.push('/enrolled-courses')}>
                            ...
                            <div className={styles.dashboardCard}>
                                        ...
                                        {latestLessons.map(lesson => (
                                            lesson?.id && lesson?.permalink && (
                                                <div
                                                    ...
                                                    role="button" tabIndex={0}
                                                    onKeyPress={(e) => (e.key === 'Enter' || e.key === ' ') && router.push(`/lessons/${lesson.permalink}`)}
                                ...
                                <button className={styles.cardActionButton} onClick={() => router.push('/lessons/completed')}>
                                    View All Completed
                                </button>
                            </div>
                    ...
                    {showTeacherSections && (
                       <>
                           <div className={styles.dashboardCard}>
                               ...
                               <button className={styles.cardActionButton} onClick={() => router.push('/teacher-dashboard')}>
                           ...
                           <div className={styles.dashboardCard}>
                               ...
                               <button className={styles.cardActionButton} onClick={() => router.push('/teacher-questions')}>
                     ...
                     {loadingDiscover ? (
                        ...
                        <section className={styles.discoverSection}>
                            <div className={styles.discoverHeader}>
                                ...
                                <Link href="/posts" className={styles.discoverSeeAllBtn}>See All Posts</Link>
                            </div>
                            <div className={styles.discoverGrid}>
                                {latestPosts.map(post => (
                                    post && (
                                        <Link href={post.permalink ? `/posts/${post.permalink}` : '#'} key={post.id} className={styles.discoverCardLink}>
                                            {post.og_image_url ? (
                        ...
                        <section className={styles.discoverSection}>
                            <div className={styles.discoverHeader}>
                                ...
                                <Link href="/learn?tab=courses" className={styles.discoverSeeAllBtn}>Explore All</Link>
                            </div>
                            <div className={styles.discoverGrid}>
                                {randomCourses.map(course => (
                                    course && (
                                         <Link href={course.permalink ? `/courses/${course.permalink}` : '#'} key={course.id} className={styles.discoverCardLink}>
                                            {course.cover_image ? (
