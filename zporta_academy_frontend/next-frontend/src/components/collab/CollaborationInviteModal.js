...
import styles from '@/styles/CollaborationInviteModal.module.css';
...
const CollaborationInviteModal = ({ isOpen, onClose, onInviteUser, courseTitle, enrollmentId }) => {
  ...
  const inviteLink = useMemo(() => {
    ...
    // Share the enrolled page. Your backend appends ?shared_token=... after invite.
    return `${origin}/courses/enrolled/${enrollmentId}`;
  }, [enrollmentId]);
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      ...
      try {
        ...
        const response = await apiClient.get(`/users/guides/?search=${encodeURIComponent(searchTerm)}`);
    ...
    <div className={styles.modalOverlay} onClick={onClose}>
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalHeader}>
          <UserPlus size={28} style={{ color: 'var(--zporta-gold)' }} />
              ...
              {users.map((user) => (
                <li key={user.id} className={styles.userListItem}>
                  ...
                  <button
                    ...
                    className={`${styles.inviteButton} ${inviteSent[user.id] ? styles.inviteButtonSent : ''}`}
        ...
        <div className={styles.linkShareSection}>
            ...
            <div className={styles.linkContainer}>
                ...
                <button type="button" onClick={handleCopyToClipboard} className="btn btn-secondary btn-sm">
                    <Copy size={18} />
                </button>
            </div>
        </div>
