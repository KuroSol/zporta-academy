...
const UsersCorrectlyModal = ({ isOpen, onClose, users, isLoading, hasNextPage, onLoadMore }) => {
    ...
    const getAvatar = (u) => {
        ...
        if (raw && !raw.startsWith('http')) return `${process.env.NEXT_PUBLIC_API_BASE_URL}${raw}`;
  ...
  return !isOpen ? null : (
    <div className="modal-overlay" role="dialog" aria-modal="true" onClick={onClose}>
      <div className="modal-content" style={{maxWidth:480}} onClick={(e)=>e.stopPropagation()}>
        ...
        {hasNextPage && (
          <div className="modal-actions">
            <button className="btn btn-secondary btn-sm" onClick={onLoadMore} disabled={isLoading}>
              {isLoading ? 'Loading…' : 'Load more'}
            </button>
          </div>
        )}
...
const QuizCard = ({
  ...
  useBodyScrollLock(showCorrectUsers || showParticipants);
      ...
      case 'sort': {
        ...
        return (
          <div className={styles.nextBridge}>
            ...
            <a className="btn btn-outline" href={href}>Open Quiz</a>
          </div>
        );
      }
      case 'dragdrop':
        ...
        {!isExpanded && (
            <>
                <div className={styles.cardTitleContainer} style={{ background: cardGradient }}>
                    {quiz.created_by && (
                        <Link href={`/guide/${quiz.created_by.username}`} className={styles.creatorInfo}>
                          <img
                ...
                <div className={styles.cardFooter}>
                    ...
                    <button onClick={() => setIsExpanded(true)} className="btn btn-primary" disabled={totalQuestions === 0}>
            ...
            {totalQuestions > 0 ? (
              <div className={styles.questionContainer}>
                ...
                <div className={styles.navigation}>
                    <button onClick={goPrev} className="btn btn-secondary" disabled={currentIndex === 0}>
                        <ChevronLeft size={18}/> Prev
                    </button>
                    {currentIndex < totalQuestions - 1 ? (
                        <button onClick={goNext} className="btn btn-primary" disabled={!isAnswerSubmittedForCurrent || isLoadingSubmission}>
                            Next <ChevronRight size={18}/>
                        </button>
                    ) : (
                        <button onClick={() => setIsExpanded(false)} className="btn btn-primary">
                            Finish
                        </button>
                    )}
                </div>
