...
export async function getServerSideProps({ params, req }) {
  ...
  try {
    ...
    const primaryLang = Array.isArray(acceptLang) ? acceptLang[0] : acceptLang.split(',')[0];
    ...
    <>
      <Head>
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          about: { '@type': 'Thing', name: tag.name, url },
          breadcrumb: {
            '@type': 'BreadcrumbList',
            itemListElement: [
              { '@type': 'ListItem', position: 1, name: 'Tags', item: `${site}/tags` },
              { '@type': 'ListItem', position: 2, name: tag.name, item: url }
            ]
          },
          mainEntity: [
            {
              '@type': 'ItemList', name: 'Posts', numberOfItems: totalPosts,
              itemListElement: (posts || []).slice(0, 10).map((p, i) => ({
                '@type': 'Article', position: i + 1, name: p.title, url: p.permalink ? `${site}/posts/${p.permalink}` : url,
                datePublished: p.published_at
              }))
            },
            {
              '@type': 'ItemList', name: 'Lessons', numberOfItems: totalLessons,
              itemListElement: (lessons || []).slice(0, 10).map((l, i) => ({
                '@type': 'CreativeWork', position: i + 1, name: l.title, url: l.permalink ? `${site}/lessons/${l.permalink}` : url,
                dateCreated: l.created_at
              }))
            },
            {
              '@type': 'ItemList', name: 'Courses', numberOfItems: totalCourses,
              itemListElement: (courses || []).slice(0, 10).map((c, i) => ({
                '@type': 'Course', position: i + 1, name: c.title, url: c.permalink ? `${site}/courses/${c.permalink}` : url,
                dateCreated: c.created_at
              }))
            },
            {
              '@type': 'ItemList', name: 'Quizzes', numberOfItems: totalQuizzes,
              itemListElement: (quizzes || []).slice(0, 10).map((q, i) => ({
                '@type': 'CreativeWork', position: i + 1, name: q.title, url: q.permalink ? `${site}/quizzes/${q.permalink}` : url,
      ...
      <div className={styles.pageWrapper}>
        <main className={styles.container}>
          <header className={styles.header}>
            ...
            {tag.description && (<p className={styles.subheading}>{tag.description}</p>)}
            <div className={styles.topRow}>
              <div className={styles.chips}>
                <Link href="#posts" className={styles.chip}><span>{totalPosts}</span> <span>{totalPosts === 1 ? 'post' : 'posts'}</span></Link>
                <Link href="#lessons" className={styles.chip}><span>{totalLessons}</span> <span>{totalLessons === 1 ? 'lesson' : 'lessons'}</span></Link>
                <Link href="#courses" className={styles.chip}><span>{totalCourses}</span> <span>{totalCourses === 1 ? 'course' : 'courses'}</span></Link>
                <Link href="#quizzes" className={styles.chip}><span>{totalQuizzes}</span> <span>{totalQuizzes === 1 ? 'quiz' : 'quizzes'}</span></Link>
              </div>
              <div className={styles.searchBar}>
                ...
                {searchTerm.trim() && (
                  <div className={styles.searchDropdown}>
                    {isSearching ? (<div className={styles.statusContainer}>Searching…</div>) : (
                      searchResults && Object.entries(searchResults).map(([category, items]) => (
                        items && items.length > 0 ? (
                          <SearchBlock key={category} title={category} items={items} />
                        ) : null
                      ))
                    )}
                  </div>
          ...
          <section id="posts">
            ...
            {posts.length === 0 ? (
              <div className={styles.statusContainer}><p>No posts found with this tag.</p></div>
            ) : (
                ...
                {posts.map((p) => (
                  <Link key={p.id} href={p.permalink ? `/posts/${p.permalink}` : '#'} className={styles.card}>
                    <div className={styles.media}>
                      {p.cover_image_url
                        ? <img src={p.cover_image_url} alt={p.title} onError={(e) => { e.target.onerror = null; e.target.src='https://placehold.co/800x600/4a5568/ffffff?text=Image'; }} />
          ...
          <section id="lessons">
            ...
            {lessons.length === 0 ? (
              <div className={styles.statusContainer}><p>No lessons found with this tag.</p></div>
            ) : (
              <div className={styles.grid}>
                {lessons.map((l) => (
                  <Link key={l.id} href={l.permalink ? `/lessons/${l.permalink}` : '#'} className={styles.card}>
                    <div className={styles.body}>
                      <h3 className={styles.title}>{l.title || 'Untitled lesson'}</h3>
                      <div className={styles.meta}>
                        <span className={styles.author}>{l.course_title || 'Lesson'}</span>
                        <span className={styles.dot}>•</span>
                        <time className={styles.date}>{formatDate(l.created_at)}</time>
                      </div>
                    </div>
                  </Link>
                ))}
          ...
          <section id="courses">
            ...
            {courses.length === 0 ? (
              <div className={styles.statusContainer}><p>No courses found with this tag.</p></div>
            ) : (
              <div className={styles.grid}>
                {courses.map((c) => (
                  <Link key={c.id} href={c.permalink ? `/courses/${c.permalink}` : '#'} className={styles.card}>
                    <div className={styles.body}>
                      <h3 className={styles.title}>{c.title || 'Untitled course'}</h3>
                      <div className={styles.meta}>
                        <span className={styles.author}>{c.subject_name || 'Course'}</span>
                        <span className={styles.dot}>•</span>
          ...
          <section id="quizzes">
            ...
            {quizzes.length === 0 ? (
              <div className={styles.statusContainer}><p>No quizzes found with this tag.</p></div>
            ) : (
              <div className={styles.grid}>
                {quizzes.map((q) => (
                  <Link key={q.id} href={q.permalink ? `/quizzes/${q.permalink}` : '#'} className={styles.card}>
                    <div className={styles.body}>
                      <h3 className={styles.title}>{q.title || 'Untitled quiz'}</h3>
                      <div className={styles.meta}>
                        <span className={styles.author}>{q.subject_name || 'Quiz'}</span>
                        <span className={styles.dot}>•</span>
                        <time className={styles.date}>{formatDate(q.created_at)}</time>
                      </div>
                    </div>
                  </Link>
                ))}
        ...
        {items.slice(0,8).map(item => (
          <li key={`${title}-${item.id}`}>
            <Link href={makeHref(item)} className={styles.searchBlockLink}>
              ...
              <span>
                {item.title || item.name || item.full_name || item.username || `Item #${item.id}`}
              </span>
            </Link>
          </li>
        ))}
